...
def dis(rva,label,n=60):
    ...
    while cur is not None and c<n:
        ...
        if cur.getMnemonicString()=="CALL":
            fl=cur.getFlows()
            if fl:
                tf=fm.getFunctionContaining(fl[0]); note=" -> %s"%(tf.getName() if tf else str(fl[0]))
        print("  +0x%06x %-40s%s"%(rr,cur.toString(),note))
        if cur.getMnemonicString() in ("RET",): break
        cur=cur.getNext(); c+=1
def dec_f(rva,label):
    f,a=ensure(rva)
    print("\n===== DECOMP %s : %s ====="%(label,str(a)))
...
dec_f(0x3b48b0,"SetCameraField (real)")
dis(0x3b48b0,"SetCameraField (real)",70)
dec_f(0x303df0,"FUN_303df0 apply-position")
print("\n=== DONE ===")
