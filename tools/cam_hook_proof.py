# -*- coding: utf-8 -*-
"""Доказательство: хук поворачивает вид. Форсируем рендер тычком Insert (хук перебивает поворот)."""
...
def tap(vk,dur=0.12):
    force=0x0001; sc=u.MapVirtualKeyW(vk,0); t0=time.time()
    while time.time()-t0<dur: u.keybd_event(vk,sc,force,0); time.sleep(0.02)
    u.keybd_event(vk,sc,force|0x0002,0)
def grab(h,save):
    ...
    img=np.frombuffer(sh.bgra,np.uint8).reshape(sh.height,sh.width,4)[:,:,:3]; cv2.imwrite(OUT+"\\"+save,img)
    return cv2.cvtColor(cv2.resize(img,(320,180)),cv2.COLOR_BGR2GRAY).astype(np.int16)
def install(ph,gb):
    if r(ph,gb+HOOK,1)[0]==0xE9: w(ph,gb+HOOK,ORIG)
    cave=int(k.VirtualAllocEx(ph,None,0x1000,0x3000,0x40)); VR=cave+0x100; ROT=cave+0x104
    ...
    w(ph,gb+HOOK,b"\xE9"+struct.pack("<i",cave-(gb+HOOK+5))+b"\x90"); return cave,VR,ROT
def main():
    h,ph,gb=attach(); force_fg(h); time.sleep(0.4)
    cont=rdw(ph,gb+0xab4f80); cam=rdw(ph,cont+0x254); neutral=rf(ph,cam+0x5b8)
    cave,VR,ROT=install(ph,gb); wdw(ph,VR,1); print("neutral=%.4f"%neutral)
    for deg,name in [(0,"proof_0.png"),(60,"proof_60.png"),(-60,"proof_m60.png")]:
        wf(ph,ROT,neutral+math.radians(deg))
        force_fg(h); tap(0x2D); time.sleep(0.15); tap(0x2E); time.sleep(0.2)  # тычки чтобы форсировать рендер
        grab(h,name); print("rot=%+d -> %s (camera+0x5b8=%.4f)"%(deg,name,rf(ph,cam+0x5b8)))
    wf(ph,ROT,neutral); wdw(ph,VR,0); w(ph,gb+HOOK,ORIG); print("хук снят")
    k.CloseHandle(ph)
